...
$BuildDir = Join-Path $env:TEMP ("usbrelay-universal-port-tool-" + [guid]::NewGuid().ToString("N"))
...
try {
    $kind = "port-tool"
    $x86Name = "USBRelay-Standard-Port-Tool-Setup-x86.exe"
    $x64Name = "USBRelay-Standard-Port-Tool-Setup-x64.exe"
    ...
    $resourceDefine = "UNIVERSAL_PORT_TOOL"
    $outputName = "USBRelay-Standard-Port-Tool-Setup-Universal.exe"
    $outputPath = Join-Path $OutputDir $outputName
    $toolDefine = "/DUNIVERSAL_PORT_TOOL"
...
Write-Host "Created:"
Write-Host (Join-Path $OutputDir "USBRelay-Standard-Port-Tool-Setup-Universal.exe")
