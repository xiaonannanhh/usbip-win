...
try {
    foreach ($kind in @("server", "client")) {
        ...
        $resourceDefine = if ($kind -eq "server") {
            "UNIVERSAL_SERVER"
        } else {
            "UNIVERSAL_CLIENT"
        }
        $outputName = if ($kind -eq "server") {
            "USBRelay-Server-Setup-Universal.exe"
        } else {
            "USBRelay-Client-Setup-Universal.exe"
        }
        $outputPath = Join-Path $OutputDir $outputName
        $clientDefine = if ($kind -eq "server") {
            "/DUNIVERSAL_SERVER"
        } else {
            "/DUNIVERSAL_CLIENT"
        }
...
Write-Host "Created:"
Write-Host (Join-Path $OutputDir "USBRelay-Server-Setup-Universal.exe")
Write-Host (Join-Path $OutputDir "USBRelay-Client-Setup-Universal.exe")
