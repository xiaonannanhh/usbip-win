...
$ServerDir = Join-Path $OutputDir "USBRelay-Standard-Server-$Architecture"
...
function New-UiExecutable {
    ...
    $resourceDefine = "UI_STANDARD_SERVER"
    ...
    Invoke-Native -FilePath $ClPath -Arguments (
        $compileFlags + @(
            "/DUSBRELAY_STANDARD_ENGINE_NO_MAIN",
            "/Fo$engineObject",
            (Join-Path $SourceRoot "userspace\src\usbrelay\usbrelay_standard_server.c")))
...
function Copy-PayloadFiles {
    ...
    $generatedFiles = @("usbrelay-standard-server-ui.exe")
    $supportFiles = @(
        "install-standard-server.cmd",
        "uninstall-standard-server.cmd",
        "README_STANDARD_SERVER_CN.txt"
    )
...
foreach ($file in @(
    ...
    $IconPath,
    (Join-Path $InstallerDir "usbrelay_standard_server_ui.c"),
    (Join-Path $InstallerDir "install-standard-server.cmd"),
    (Join-Path $InstallerDir "uninstall-standard-server.cmd"),
    (Join-Path $InstallerDir "README_STANDARD_SERVER_CN.txt"),
    (Join-Path $SourceRoot "userspace\src\usbrelay\usbrelay_standard_server.c"))) {
...
try {
    New-UiExecutable -Kind "standard-server" `
        -SourceFile (Join-Path $InstallerDir "usbrelay_standard_server_ui.c") `
        -OutputDirectory $ServerDir `
        -OutputName "usbrelay-standard-server-ui.exe"
    Copy-PayloadFiles
    $serverCab = New-PayloadCab -Name "standard-server-payload" `
        -PayloadDir $ServerDir
    New-SetupExe -Kind "standard-server" -PayloadCab $serverCab `
        -OutputName "USBRelay-Standard-Server-Setup-$Architecture.exe"
} finally {
...
Write-Host "Created:"
Write-Host (Join-Path $OutputDir "USBRelay-Standard-Server-Setup-$Architecture.exe")
